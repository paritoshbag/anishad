...
export default function ListSlider() {
    const { data, loading, mutate } = useDataFetching('/api/admin/slider');
                    ...
                    <Stack flexDir={'row'} gap={2}>
                        <Button
                            ...
                            onClick={() => {
                                ...
                                // EditTest(row.id)
                                router.push(`/dashboard/add-slider/list/edit/${row.id}`)
                            }}
            ...
            <Sidebar>
                <Stack>
                    <Stack flexDir={'row'}>
                        ...
                        />
                        <Button colorScheme='green' onClick={() => router.push('/dashboard/add-slider')}>New</Button>
                    </Stack>
                    {loading ? (
                        <div>Loading...</div>
                    ) : (
